...
### 2026-06-23 新增：考试模拟模式（v1.3.0）

**功能目标**：为 OJ 系统新增考试模拟模式，支持定时作答、暂停计时、自动交卷和结果汇总。

**后端新增**：

- **Exam 表**（`backend/models.py`）：
  - `id`, `user_id` (FK), `title`, `problem_ids` (JSONB), `duration_minutes`, `start_time`, `end_time`, `total_paused_seconds`, `status` (enum: pending/in_progress/paused/completed), `created_at`, `updated_at`
- **Exam 接口**（`backend/routers/exams.py`）：
  - `POST /api/exams` — 创建考试
  - `POST /api/exams/{id}/start` — 开始考试（写入 start_time）
  - `POST /api/exams/{id}/pause` — 暂停考试（用于前端暂停遮罩期间）
  - `POST /api/exams/{id}/resume` — 恢复考试（累计暂停时长）
  - `POST /api/exams/{id}/submit` — 交卷（写入 end_time，status → completed）
  - `GET /api/exams/{id}/results` — 查看考试结果（总分、每题状态、得分）
  - `GET /api/exams` — 列出当前用户所有考试记录
- 暂停计时通过 `updated_at` 时间戳差值累计到 `total_paused_seconds`
- 每道题按 problem_ids 顺序平分 100 分，AC 则得分，否则 0 分

**前端新增**（`frontend/src/pages/Exam/`）：

- **ExamCreatePage** (`/exam/create`)：标题 + 题目 slug 列表 + 时长表单，创建后跳转到考试页
- **ExamPage** (`/exam/:examId`)：核心考试工作台
  - 顶部倒计时条（red < 5 min warning），暂停/交卷按钮
  - 多题目 Tab 切换，每题独立状态指示灯（绿=AC, 黄=已提交, 灰=未做）
  - 复用 `WorkspacePanels` 组件（完整代码编辑 + 提交 + 自测 + 提交历史）
  - 每题草稿独立 localStorage（按 examId + problemId 隔离）
  - 暂停遮罩层：半透明黑色覆盖 + 题目不可见 + 禁止编辑
  - 自动交卷：倒计时归零自动调用 submit API
  - 手动交卷：需二次确认弹窗
- **ExamResultsPage** (`/exam/:examId/results`)：考试结果汇总
  - 总分 / 满分展示 + 进度条
  - 实际用时（已扣除暂停时间）
  - 每题一行：题号、slug（可跳转题目详情）、状态、得分
- **Navbar**：新增「考试」入口（登录后可见）

**路由**：所有考试路由使用 `ProtectedRoute`（需登录）

**文档**：新增 `EXAM_SPEC.md`（考试模式完整使用教程）

**当前考试**：id=3 "模拟一：W1-W6 综合诊断（6/24场）"，题目 5 道，时长 90 分钟，状态 pending

**Dashboard 入口**：首页题目列表右上角新增「考试模式」按钮（登录后可见）
...
### 2026-06-04 新增：服务器部署指南文档

**新增文件**：`D:\OJ\SERVER_GUIDE.md`

**目的**：为其他 AI 或开发者提供在 Ubuntu 服务器上部署新项目的指南，确保不影响现有 OJ 系统。

**内容包括**：
- 服务器 SSH 连接信息（IP、用户名、密钥路径）
- 现有服务清单（端口、容器名、目录结构）
- 端口使用情况与推荐新端口
- 数据库隔离策略（PostgreSQL 新 database）
- Redis 隔离策略（使用不同 db 编号）
- Docker Compose 部署模板
- Nginx 配置方案（路径前缀 / 新域名）
- 红线清单（不可修改的文件和配置）
